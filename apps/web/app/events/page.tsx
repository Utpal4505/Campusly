  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-between">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 py-8">
        ...
        {/* Events Grid (Option 1: Modern Unified Luma/Devpost Cards) */}
        {filteredEvents.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 gap-5 mb-12">
            {filteredEvents.map((event) => (
              <div
                key={event.id}
                className="rounded-3xl border border-border/80 bg-card hover:border-primary/40 hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col justify-between group relative"
              >
                {/* Subtle Ambient Glow Header */}
                <div className={`absolute top-0 left-0 right-0 h-32 bg-gradient-to-b ${event.gradient} opacity-25 pointer-events-none`} />
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between gap-2 mb-4">
                      <div className="flex items-center gap-2">
                        <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-0.5 rounded-full border ${event.categoryBadge}`}>
                          {event.category}
                        </span>
                      ...
                      <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-muted/60 text-foreground border border-border/60 flex items-center gap-1.5 shadow-2xs">
                    ...
                    <div className="flex items-start gap-3.5 mb-3">
                      {/* Modern Date Tile */}
                      <div className="w-12 h-14 rounded-2xl bg-muted/50 border border-border/80 flex flex-col items-center justify-center shrink-0 text-center overflow-hidden shadow-2xs">
                        <div className="w-full bg-primary/15 text-primary text-[10px] font-black uppercase tracking-wider py-0.5 border-b border-border/50">
                    ...
                    <p className="text-xs text-foreground/80 leading-relaxed line-clamp-2 mb-3.5 font-normal">
                      {event.subtitle}
                    </p>

                    {/* Metadata Chips: Location + Timing */}
                    <div className="flex flex-wrap items-center gap-2 mb-3.5 text-xs">
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-muted/40 text-foreground/90 font-medium border border-border/50 text-[11px]">
                  ...
                  <div className="pt-3.5 border-t border-border/60 flex items-center justify-between gap-3 mt-auto">
                    <div className="flex items-baseline gap-1.5">
                      ...
                      <span className="text-sm font-extrabold text-foreground">
                        {event.isFree ? (
                          <span className="text-emerald-500">Free Pass</span>
                        ) : (
                          event.entryFee
                        )}
                      </span>
